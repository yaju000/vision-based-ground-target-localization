...
class aoa_info(object):
    ...
    def visual_r(self, r, v, hd, ob, roll, pitch, yaw):
        ...
        return angle_a_w0, pro_vector, pro_vector_0
    ...
    def iteration(self, event):
        ...
        if self.mode == 0 : 
            if ([self.u, self.v]!=[None, None]) and ([self.u, self.v]!=[0, 0]):
                ...
                angle_a_w0, pro_vector,  pro_vector_0 = self.visual_r(150,self.vel,self.hd_deg, ob_point, self.roll, self.pitch, self.yaw)
                ...
                acc_cmd = np.square(self.vel)/150 + k*sin_gamma
                roll_cmd = np.arctan(acc_cmd/9.81)
                pitch_cmd = self.height_control(self.gps_pose, self.vel, 0.1)
                ...
                if len(azimuth) == 0:
                    ...
                    angle_a_w0, pro_vector,  pro_vector_0 = self.visual_r(150,self.vel,self.hd_deg, [self.gps_pose[1], self.gps_pose[0], -self.gps_pose[2]], self.roll, self.pitch, self.yaw)
                    est_vector = [self.est_vector_n, self.est_vector_e]
